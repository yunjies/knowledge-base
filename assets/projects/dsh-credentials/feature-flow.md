...
flowchart TB
  ...
  ACTION -->|"同步到另一台机器"| SYSELECT["选目标机与凭证集"]
  WRITE --> LIST
  UNSET --> LIST
  SYSELECT --> SYSCMD{"生成命令的条件是否成立"}
  SYSCMD -->|"成立"| SYSPASTE(["产出可粘贴的命令"])
  SYSCMD -->|"主机名不合法或选中项在本机无值"| SYSDENY(["拒绝并写明原因"])
...
用户在设置页的「同步到另一台机器」里选要同步的凭证集与目标主机。展开该面板时另调 `sync-candidates` 取候选集（与 `LIST` 同源的服务对象，但是独立的一次读取，因此本节点的输入不含 `LIST` 的任何输出）：**只有 `state` 为 `ready` 的条目前可选择**——把一个本机无值的条目列进命令，会让用户在粘贴之后才在对端失败，而那时他已经离开这一页。`SSH_PRIVATE_KEY` 这类高敏感项带标记，界面上与 API key 区别显示。

**输入**

- `ROWS`：已渲染的行；来源为 `GRID`。当前状态决定哪些条目可选。
- `USER_ACTION`：用户的勾选与主机名输入；来源为界面交互。

**输出**

- `SYNC_INTENT`：目标主机与选中的 id 集；去向为 `SYSCMD`。

## SYSCMD

生成条件的判定，是本功能的**唯一分叉点**，也是全部安全性质的落点。成立则走 `SYSPASTE`，不成立走 `SYSDENY`。

成立需同时满足三项：目标主机非空、主机名匹配 `^[A-Za-z0-9._-]+(@[A-Za-z0-9._-]+)?$`、且每个选中项在本机都持有非空值。主机名带 shell 元字符时**整体拒绝而非转义后使用**：主机名有确定的文法和边界，接受文法外的东西会把一条可粘贴命令变成注入面。

生成结果里**不含任何值**。命令只由 ref 名与目标主机拼成，值由 `SYSPASTE` 那条命令在执行时经 ssh 管道传输。

**输入**

- `SYNC_INTENT`：目标主机与选中的 id 集；来源为 `SYSELECT`。

**输出**

- `SYNC_PLAN`：命令、ref 名列表、敏感标记；去向为 `SYSPASTE`。
- `SYNC_REFUSAL`：拒绝原因；去向为 `SYSDENY`。

## SYSPASTE

用户拿到一条命令，在**持有这些凭证的那台机器**上执行。节点本身不在本插件进程内发生——它是用户与 shell 的动作，本插件产出的是这条命令。

命令形态为 `export | ssh -T <host> import`：本机侧写出载荷，经 ssh 的 stdin 送到对端，对端由 `import` 读入并逐条写入。`-T` 是刻意的：对端分配 tty 正是会把载荷回显进终端滚动缓冲的那件事。

对端的 `import` 校验载荷版本、按 ref 逐条写 seam，并把**部分失败按失败处理**：能写的写上、写不上的列出原因并给出非零结论。一个缺失的凭证不会立刻报错，等到报错时通常已经很贵。

本节点是终止态：命令执行完，值已在目标机落库。

**输入**

- `SYNC_PLAN`：命令与 ref 名列表；来源为 `SYSCMD`。
- `TARGET_SHELL`：对端执行环境；来源为目标机。

**输出**

- `REMOTE_APPLIED`：对端写入结果与摘要；去向为目标机的凭证库。

**失败模式**

- 对端未装本插件：`import` 不存在，ssh 返回非零并报 command not found。
- 对端 seam 只读（该 ref 被启动环境遮蔽）：逐条透传 seam 原话，其余条目照常写入，整体结论为失败。
- 载荷与命令指定的 ref 集不一致：拒绝且不写任何一条——两者由两次独立调用产生，不一致是可表示的，必须挡住。

## SYSDENY

生成条件不成立时的终止出口：界面提示原因（主机名不合法 / 选中项在本机无值 / 未选任何项），不产出任何命令。本节点不出边——用户能改的是输入，不是运行环境。

**输入**

- `SYNC_REFUSAL`：拒绝原因；来源为 `SYSCMD`。
